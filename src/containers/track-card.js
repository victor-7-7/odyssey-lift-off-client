...
const TrackCard = ({ track }) => {
  ...
  // element is an object with information about the mutation: loading, error
  // and data. This component doesn't need it, so we don't have to extract it.
  const [incrementTrackViews] = useMutation(
    INCREMENT_TRACK_VIEWS, {
      // This id has already been destructured for us at the top from the track prop.
      variables: { trackId: id },
      // The onCompleted property is a callback function that will run
      // when the mutation successfully completes, and it has access to
      // the response that comes back. We'll log the response to the browser console.
      onCompleted: (data) => {
        console.log(data);
      },
    });
